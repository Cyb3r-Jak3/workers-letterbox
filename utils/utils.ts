...
export async function ReadKeyFromFile(keyFile: File): Promise<{ key?: Key; success: boolean }> {
	let key: Key;
	try {
		...
		try {
			key = await readKey({ binaryKey: new Uint8Array(await keyFile.arrayBuffer()) });
			return { key, success: true };
		} catch {
...
export async function ReadMessageFromFile(
	...
	try {
		message = await readMessage({ armoredMessage: await messageFile.text() });
